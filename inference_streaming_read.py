...
def detect_video(
    ...
    msgs_v_avg = soft_msgs.mean(dim=0)  # Average the predictions across all frames
    soft_msgs = (soft_msgs > 0).to(int)
...
def main(args):
    ...
    result = binary_to_ascii(binary_normalized)
    print(result)
...
def binary_to_ascii(bits):
    ...
    binary_list = [bit_string[i:i+8] for i in range(0, len(bit_string), 8)]
